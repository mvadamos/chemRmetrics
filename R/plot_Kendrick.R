...
plot_Kendrick <- function(df, unit_mass, unit_name, SNR, scale, trunc = FALSE, max_mass, method = 'MAD', halfWindowSize = 10, peak_pick = 'auto', ymin, ymax, xmin, xmax){
  ...
  for (i in 1:nrow(df)){
    ...
    pdf(paste('KMD Plots/', paste(unit_name, paste(paste('Sample #', df$`Sample ID`[i], sep = ''), paste(paste('KMD Plot', rowname(df)[i], sep = ''), '.pdf', sep = ''), sep = ' '), sep = '_'), sep = ''))
    ...
    pdf(paste('KMR Plots/', paste(unit_name, paste(paste('Sample #', df$`Sample ID`[i], sep = ''), paste(paste('KMR Plot', rowname(df)[i], sep = ''), '.pdf', sep = ''), sep = ' '), sep = '_'), sep = ''))
    ...
    htmlwidgets::saveWidget(fig_2D_KMD, file = paste('KMD Plots/', paste(unit_name, paste(paste('Sample #', df$`Sample ID`[i], sep = ''), paste(paste('KMD Plot', rowname(df)[i], sep = ''), '.html', sep = ''), sep = ' '), sep = '_'), sep = ''))
    ...
    htmlwidgets::saveWidget(fig_2D_KMR, file = paste('KMR Plots/', paste(unit_name, paste(paste('Sample #', df$`Sample ID`[i], sep = ''), paste(paste('KMR Plot', rowname(df)[i], sep = ''), '.html', sep = ''), sep = ' '), sep = '_'), sep = ''))

    # Export KMR Data '.csv'
    KMR_data <- cbind(peaks_mass[, 1], peaks_KMR)
    KMR_data <- cbind(KMR_data, peaks_intensity[, 2]/2)

    write.csv(KMR_data, paste(paste(paste('KMR Plots/KMR Data Sample #_', df$`Sample ID`[i], sep = ''), rowname(df)[i], sep = ''),  '.csv', sep = ''))
  }
}
