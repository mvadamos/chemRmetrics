...
plot_Kendrick <- function(df, unit_mass, unit_name, SNR, scale, trunc = FALSE, max_mass, method = 'MAD', halfWindowSize = 10, peak_pick = 'auto', ymin, ymax){
  ...
  for (i in 1:nrow(df)){
    ...
    for (r in 1:nrow(peaks_mass)){
      peaks_KMR[r, 1] <- peaks_mass[r, 1] %% unit_mass
    }
    ...
    htmlwidgets::saveWidget(fig_2D_KMD, file = paste('KMD Plots/', paste(unit_name, paste(paste('Sample #', df$`Sample ID`[i], sep = ''), 'KMD Plot.html', sep = ' '), sep = '_'), sep = ''))
    ...
    htmlwidgets::saveWidget(fig_2D_KMR, file = paste('KMR Plots/', paste(unit_name, paste(paste('Sample #', df$`Sample ID`[i], sep = ''), 'KMR Plot.html', sep = ' '), sep = '_'), sep = ''))

    # Export KMR Data '.csv'
    KMR_data <- cbind(peaks_mass[, 1], peaks_KMR)
    KMR_data <- cbind(KMR_data, peaks_intensity[, 2]/2)

    write.csv(KMR_data, paste(paste('KMR Plots/KMR Data Sample #', df$`Sample ID`[i], sep = ''), '.csv', sep = ''))
  }
}
